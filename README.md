...
在服务器上以 root 权限直接运行以下命令，脚本会先打开一个菜单：
...
其中只有 `安装` 会执行完整部署流程。选择 `安装` 后，脚本会根据提示输入：
- 解析到该服务器的域名。
- Hysteria2 和 TUIC 的端口及密码。
- 选择 BBR 版本（推荐 BBR v1 以获得最大兼容性）。

如果在安装过程中选择 BBR v3，脚本会先完成内核相关配置并提示重启。服务器重启后，请再次运行脚本，并在菜单中再次选择 `安装` 继续后续部署。

其余 `启动`、`停止`、`重启`、`状态`、`卸载` 选项仅用于管理已安装的 `sing-box` 服务。

## 注意事项
- **端口**: 请确保服务器防火墙（如 `ufw`, `firewalld` 或安全组）已放行您配置的端口。
- **BBR v3**: 选择 BBR v3 会自动安装第三方内核并修改引导设置，操作完成后**必须重启服务器**以生效；重启后请重新运行脚本并选择 `安装` 继续。
